import { CollectionReference, DocumentSnapshot, Query } from "firebase/firestore";
...
export function QueueTableRowsGrouped({queue, collectionRef, groupBy, tableConfig}: GroupedQueueItemsProps) {
  ...
  const [selectedQueueItem, setSelectedQueueItem] = useState<DocumentSnapshot<QueueItem> | undefined>(undefined)
  const handleQueueItemEditDialogOpen = (id: string) => {
    const item = itemCollection?.docs.find((val) => val.id === id)
    if (item) {
      ...
      {itemCollection && items && groupedItems && (
        sortedGroupKeys.map((group) => {
          ...
          for (const row of groupedItems[group]) {
            elms.push(
              <TableRow key={row.id} hover onClick={() => handleQueueItemEditDialogOpen(row.id)}>
                {tableConfig.map((col) => {
